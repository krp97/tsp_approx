    ...
    : a_matrix_{std::vector<std::vector<int>>()}
{
    ...
    if (!f.run()) throw std::invalid_argument(f.what());
}
...
{
    if (a_matrix_ == rhs.a_matrix_) return true;
    return false;
}
...
int Adjacency_Matrix::find_longest_number() const
{
    ...
    for (auto& rows : a_matrix_) {
        for (auto& cols : rows) {
            if ((digits = get_digits(cols)) > most_digits) most_digits = digits;
        }
    }
    return most_digits;
}

void Adjacency_Matrix::rows_to_string(std::string& output,
...
void Adjacency_Matrix::load_data(std::fstream& file)
{
    ...
    for (auto& rows : a_matrix_) {
        std::getline(file, line);
        if (!file.eof()) rows = line_to_vec(line);
    }
}
...
std::vector<int> Adjacency_Matrix::line_to_vec(std::string& line)
{
    ...
    int value;
    while (iss >> value) output.push_back(value);
...
std::vector<int> Adjacency_Matrix::get_neighbours(int node) const
{
    ...
    for (size_t i{0}; i < a_matrix_[node].size(); ++i) {
        if (a_matrix_[node][i] != 0) output.push_back(i);
    }
    return output;
}